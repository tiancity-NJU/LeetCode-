...
/*
By backtracking.
flag用来存储是否访问过，pos为word的字母索引。
author: ypz
*/

class Solution {
public:
    bool backTracking(vector<vector<char>>& board, vector<vector<bool>>& flag, int pos, int i, int j, string word) {
        if(pos == word.size()) return true;
        if(i < 0 || i >= board.size() || j < 0 || j >= board[0].size()) return false;
        if(flag[i][j]) return false;
        if(board[i][j] != word[pos]) return false;
        flag[i][j] = true;
        bool ans = backTracking(board, flag, pos+1, i+1, j, word) || backTracking(board, flag, pos+1, i-1, j, word) || backTracking(board, flag, pos+1, i, j+1, word) || backTracking(board, flag, pos+1, i, j-1, word);
        flag[i][j] = false;
        return ans;
    }
    ...
    bool exist(vector<vector<char>>& board, string word) {
        ...
        for(int i = 0; i < board.size(); i++) {
            for(int j = 0; j < board[0].size(); j++) {
                if(backTracking(board, flag, 0, i, j, word)) return true;
            }
        }
        return false;
    }
